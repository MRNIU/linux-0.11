...
#include <string.h> //字符串头文件,主要定义了一些有关字符串操作的嵌入函数.主要是用了其中的 memset() 函数
...
// 输入:%0-eax(返回值),%1-eax(0);%2-nr;%3-(addr),addr 的内容
...
// 复位指定逻辑块 block 的逻辑块位图比特位.参数:dev 是设备号,block 是逻辑块号(盘块号).
void free_block(int dev,int block){
  struct super_block *sb;
  struct buffer_head *bh;
// 取指定设备 dev 的超级块,如果指定设备不存在,则出错死机.
  if(!(sb=get_super(dev)))  panic("trying to free block on nonexistent device");
// 若逻辑块号小于第一个逻辑块号或者大于设备上总逻辑块数,则出错,死机
...
// 该段代码的主要用途是:如果该逻辑块当前存在于高速缓冲中,就释放对应的缓冲块.
  ...
  if(bh){
    if(bh->b_count!=1){
      printk("trying to free block (%04:%d),count=%d\n",dev,block,bh->b_count);
      return;
    }
...
// 从设备 dev 取超级块,如果指定设备不存在,则出错死机
  if(!(sb=get_super(dev)))  panic("trying to get new block from nonexistant device");
// 扫描逻辑块位图,寻找第一个 0 比特位,寻找空闲逻辑块,获取放置该逻辑块的块号
  j=8192;
  for(i=0;i<8;i++)
    if(bh=sb->s_zmap[i])
      if((j=find_first_zero(bh->b_data))<8192)
        break;
// 如果全部扫描完还没找到(i>=8 或 j>=8192)或者位图所在的缓冲区块无效(bh=NULL)则返回 0,退出
// (没有空闲逻辑块)
  if(i>=8||!bh||j>=8192)  return 0;
// 设置新逻辑块对应逻辑块位图中的比特位,若鬼影比特位已经置位,则出错,死机
  if(set_bit(j,bh->b_data)) panic("new_block:bit already set");
// 置对应缓冲区块的已修改标志.如果新逻辑块大于该设备上的总逻辑块数,则说明指定逻辑块在对应设备上
// 不存在.申请失败,返回 0,退出.
  bh->b_dirt=1;
...
// 释放指定的 i 节点.复位对应节点 i 位图比特位.
void free_inode(struct m_inode *inode){
  struct super_block * sb;
  struct buffer_head * bh;
// 如果 i 节点指针 =NULL,则退出
  if(!inode)  return;
// 如果 i 节点上的设备号字段为 0,说明该节点无用,则用 0 清空对应 i 节点索镇内存区,并返回.
  if(!inode->i_dev){
...
// 如果文件目录连接数不为 0,则表示还有其它文件目录向使用该节点,不应释放,而应该放回等待.
  if(inode->i_nlinks) panic("trying to free inode with links");
// 取 i 节点所在设备的超级块,测试设备是否存在
  if(!(sb=get_super(inode->i_dev))) panic("trying to free inode on nonexistent device");
...
// 从内存 i 节点表(inode_table)中获取一个空闲 i 节点项(inode)
  ...
  if(!(sb=get_super(dev)))  panic("new_inode with unknown device"); // 读取指定设备的超级块结构
// 扫描 i 节点位图,寻找第一个 0 比特位,寻找空闲节点,获取放置该 i 节点的节点号
  j=8192;
  for(i=0;i<8;i++)
    if(bh=sb->s_imap[i])
      if((j=find_first_zero(bh->b_data))<8192)
        break;
// 若全部扫描完还没找到,或者位图所在缓冲块无效(bh=NULL)则返回 0,退出(没有空闲 i 节点)
...
// 置位对应新 i 节点的 i 节点位图相应比特位,如果已经置位,则出错
  ...
  bh->b_dirt=1; // 置 i 节点位图所在缓冲区已修改标志
// 初始化该 i 节点结构
