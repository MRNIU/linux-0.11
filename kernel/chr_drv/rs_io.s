...
_rs1_interrupt: # 串行端口 1 中断处理程序入口点
	pushl $_table_list+8 # tty 表中对应串口 1 的读写缓冲指针的地址入栈(tty_io.c,100)
	jmp rs_int
.align 2
...
rs_int:
	...
	pop %ds  # 由于这是一个中断程序，我们不知道 ds 是否正确，所以加载它们(让 ds、es 指向内核数据段)
	pushl $0x10
	pop %es
	movl 24(%esp),%edx # 将缓冲区队列指针地址存入 edx 寄存器，即 33 或 37 行上最先压入堆栈的地址
	movl (%edx),%edx # 取读队列指针(地址)->edx
	movl rs_addr(%edx),%edx  # 取串口 1 的端口号->edx
	addl $2,%edx # interrupt ident. reg   edx 指向中断标识寄存器
rep_int:  # 中断标识寄存器端口是 0x3fa(0x2fa),参见上节列表后信息
	xorl %eax,%eax # eax 清零
...
read_char:
	...
	movl (%ecx),%ecx # read-queue 读缓冲队列结构地址->ecx
...
write_char:
	...
	subl tail(%ecx),%ebx # 头指针-尾指针=队列中的字符数
