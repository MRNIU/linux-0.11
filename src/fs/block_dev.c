...
// count-要传送的字节数。
// 对内核来说，写操作是高速缓冲区中写入数据，什么时候数据最终写入设备是由高速缓冲管理程序决定
...
int block_write(int dev, long *pos, char *buf, int count) {
    ...
    while (count > 0) {
        ...
        // 的字节，因此这里需要置 offset 为零。
        p = offset + bh->b_data;
        offset = 0;
        // 将文件中偏移指针前移已写字数，累加已写字节数 chars。传送计数值减去此次已传送字节数。
        *pos += chars;
        written += chars;
        count -= chars;
        // 从用户缓冲区复制到 chars 字节到 p 指向的高速缓冲区中开始写入的位置
        while (chars-- > 0)
            *(p++) = get_fs_byte(buf++);
...
int block_read(int dev, unsigned long *pos, char *buf, int count) {
    ...
    while (count > 0) {
        ...
        if (chars > count)
            chars = count;
        // 读入需要的数据块，并预读下两块数据，如果读操作出错，则返回已读字节数，如果没有读入任何字节，
        // 则返回出错号。然后将块号递增 1
        if (!(bh = breada(dev, block, block + 1, block + 2, -1)))
            return read ? read : -EIO;
        block++;
        // p 指向从设备读出数据块中需要读取的开始位置。若最后需要读区的数据不足一块，则需从块开始
        // 读取所需的字节，因此这里需将 offset 置零
        p = offset + bh->b_data;
        offset = 0;
        // 将文件中偏移指针前移已读出字节数 chars。累加已读字节数。传送计数值减去此次以传送字节数
        *pos += chars;
        read += chars;
        count -= chars;
