...
// 以 #! 开始的检测代码是由 tytso 实现的

// Demand-loading implemented 01.12.91-no need to read anything but the header
// into memory. The inode of the executable is put into "current->executable",
// and page faults do the actual loading. Clean
//
// Once more I can proudly say that linux stood up to being changed: it was less
// than 2 hours work to get demand-loading completely implemented.
// 需求时家在是于 1991 年 12 月 1 日实现的-只需将执行文件头部分读进内存而无须将整个执行文件都
// 加载进内存。执行文件的 i 节点被放在当前进程的可执行字段中(current->executable), 而页
// 异常会进行执行文件的实际加载操作以及清理工作。
// 我可以再一次自豪地说，Linux 经得起修改：只用了不到 2 小时的工作时间就完全实现了需求加载处理。

#include <errno.h>  // 错误号头文件。包含系统中各种出错号.(Linus 从 MINIX 中引进的)
#include <string.h> // 字符串头文件。主要定义了一些有关字符串操作的嵌入函数。
#include <sys/types.h>  // 类型头文件。定义了基本的系统数据类型。
#include <sys/stat.h> // 文件状态头文件。含有文件或文件系统状态结构 stat{} 和常量。
#include <a.out.h>  // a.out 头文件。定义了有关 a.out 执行文件格式和一些宏。
...
// MAX_ARG_PAGES 定义了新程序分配给参数和环境变量使用的最大内存页数。
// 32 页内存应该足够了，这是的环境和参数(env+arg)空间的综合达到 128KB！
...
static unsigned long change_ldt(unsigned long text_size, unsigned long *page) {
    ...
    // 末端。是调用函数 put_page() 进行操作的(mm/memory.c,22222)
...
int do_execve(unsigned long *eip, long tmp, char *filename, char **argv, char **envp) {
    ...
    // 执行文件必须是常规文件。若不是常规文件则置出错返回码，跳转到 exec_error2(第 458 行)。
    restart_interp:
    ...
    // 长度 a_trsize 不等于0、或者数据重定位信息长度不等于 0、或者代码段+ 数据段+ 堆段长度超过 50MB、
    ...
    // 清复位所有信号处理句柄。但对于 SIG_IGN 句柄不能复位，因此在 421 与 422 行之间需添加一条
