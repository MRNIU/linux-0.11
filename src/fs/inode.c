...
// 对指定的 i 节点解锁.复位节点 i 的锁定标志,并明确地唤醒等待此 i 节点的进程
static inline void unlock_inode(struct m_inode *inode) {
    inode->i_lock = 0;
    wake_up(&inode->i_wait);
}

// 释放内存中设备 dev 的所有 i 节点.扫描内存中 i 节点表,弱势指定设备使用的 i 节点就释放之.
...
// 同步所有 i 节点.同步内存与设备上的所有 i 节点信息
void sync_inodes(void) {
    int i;
    struct m_inode *inode;
    inode = 0 + inode_table;  // 让指针首先指向 i 节点表指针数组首项
    for (i = 0; i < NR_INODE; i++, inode++) {  // 扫描该 i 节点表指针数组
        wait_on_inode(inode); // 等待该 i 节点可用(解锁)
        if (inode->i_dirt && !inode->i_pipe) // 如果该 i 节点已修改且不是管道节点,则写盘
            write_inode(inode);
    }
}

// 文件数据块映射到盘块的处理操作.(block 位图处理函数,bmap-block map)
// 参数:inode-文件的 i 节点;block-文件中俄数据块号;create-创建标志
// 如果创建标志置位,则在对应逻辑块不存在时就申请新磁盘块.返回设备上的逻辑块号(盘块号)
static int _bmap(struct m_inode *inode, int block, int create) {
    struct buffer_head *bh;
    int i;
    if (block < 0) panic("_bmap:block<0"); // 如果块号小于 0,则死机
// 如果块号大于直接数+间接块数+二次间接块数,超出文件系统表示范围,则死机
    if (block >= 7 + 512 + 512 * 512) panic("_bmap:block>big");
    if (block < 7) {  // 如果该块号小于 7,则使用直接块表示
...
// 如果该块号>=7,并且小于 7+512,则说明是一次间接块.下面对一次间接块进行处理
    block -= 7;
    if (block < 512) {
// 若是创建,且该 i 节点中对应间接块字段为 0,表明文件时首次使用间接块,则需申请一磁盘块用于存放
// 间接块信息,并将此磁盘块号填入间接块字段中.然后设置 i 节点已修改标志和修改时间
...
// 如果是创建并且间接块的第 block 项中的逻辑块号为 0 的话,则申请一磁盘块(逻辑块),并让间接块中的
// 第 block 项等于该新逻辑块块号.然后置位间接块的已修改标志
        if (create && !i)
...
// 程序运行到此,表明数据块是二次间接块,处理过程与一次间接块类似.下面是对二次间接块的处理.将 block
// 再见去间接块所容纳的块数(512)
    block -= 512;
...
// 若此时 i 节点二次间接块字段为 0,表明申请磁盘块失败,返回 0退出
    if (!inode->i_zone[8]) return 0;
    if (!(bh = bread(inode->i_dev, inode->i_zone[8]))) return 0; // 读取该二次间接块的一级块
// 取该二次间接块的一级块上第(block/512)项中的逻辑块号
    i = ((unsigned short *) bh->b_data)[block >> 9];
// 如果是创建并且二次间接块的一级块上第(block/512)项中的逻辑块号为 0 的话,则需申请一磁盘块(逻辑块)
// 作为二次间接块的二级块,并让二次间接块的一级块中第(block/512)项等于该二级块的块号.然后置位二次间接块
// 的一级块已修改标志.并释放二次间接块的一级块.
    if (create && !i)
        if (i = new_block(inode->i_dev)) {
    ...
    brelse(bh);
    if (!i) return 0; // 如果二次间接块的二级块块号为 0,表示申请磁盘块失败,返回 0 退出.
    if (!(bh = bread(inode->i_dev, i))) return 0; //  读取二次间接块的二级块
// 取该二级块上第 block 项中的逻辑块号.(与 511 进行与操作是为了限定 block 值不超过 511)
    i = ((unsigned short *) bh->b_data)[block & 511];
// 若是创建,且二级块的第 block 项中逻辑块号为 0,则申请一磁盘块(逻辑块),作为最终存放数据信息的
// 块.并让二级块中的第 block 项等于该新逻辑块块号(i).然后置位二级块的已修改标志.
    if (create && !i)
        if (i = new_block(inode->i_dev)) {
...
// 根据 i 节点信息取文件数据块 block 在设备上已赢得逻辑块号
...
// 如果是管道 i 节点,则唤醒等待该管道的进程,引用次数减 1,如果还有引用则返回.否则释放管道占用的
// 内存页面,并复位该节点的引用计数值、已修改标志和管道标志,并返回.对于 pipe 节点,inode->i_size
// 存着物理内存页地址.参见 get_pipe_inode(),第 257,260 行
    if (inode->i_pipe) {
        wake_up(&inode->i_wait);
    ...
    if (!inode->i_dev) {  // 若 i 节点对应设备号=0,则将此节点引用计数递减 1,返回
        inode->i_count--;
        return;
    }
...
// 如果 i 节点已作过修改,则更新该 i 节点,并等待该 i 节点解锁
    if (inode->i_dirt) {
        write_inode(inode);
        wait_on_inode(inode);
        goto repeat;
...
// 寻找引用计数 count 为 0 的节点,并将其写盘后清零,返回其指针.
struct m_inode *get_empty_inode(void) {
    ...
    do {
        inode = NULL;
        for (i = NR_INODE; i; i--) {
// 如果 last_inode 已经指向 i 节点表的最后 1 项后,则让其重新指向 i 节点表开始处.
...
// 如果没有找到空闲 i 节点(inode=NULL),则将整个 i 节点表打印出来供调试使用,并死机.
        ...
        wait_on_inode(inode); // 等待该 i 节点解锁(如果又被上锁的话)
// 如果该 i 节点已修改标志被职位的话,则将该 i 节点刷新,并等待该 i 节点解锁
        while (inode->i_dirt) {
            write_inode(inode);
            wait_on_inode(inode);
        }
    } while (inode->i_count); // 如果 i 姐弟啊又被其它进程占用的话,则重新寻找空闲 i 节点.
...
// 首先扫描 i 节点表,寻找一个空闲 i 节点项,然后取得一页空闲内存供管道使用.再将得到的 i 节点的
// 引用计数置为 2(读者和写者),初始化管道头和尾,置 i 节点的管道类型表示
struct m_inode *get_pipe_inode(void) {
    struct m_inode *inode;
    if (!(inode = get_empty_inode())) return NULL;  // 如果找不到空闲 i 节点则返回 NULL
    if (!(inode->i_size = get_free_page())) { // 节点的 i_size 字段指向缓冲区
        inode->i_count = 0; // 如果已没有空闲内存,则
        return inode; // 释放该 i 节点,并返回  NULL
    }
...
// 找到指定设备号和节点号的 i 节点,等待该节点解锁(如果已上锁的话)
        wait_on_inode(inode);
// 在等待该节点解锁的阶段,节点表可能会发生变化,所以在此判断,如果发生了变化,则再次扫描整个 i 节点表
        if (inode->i_dev != dev || inode->i_num != nr) {
            inode = inode_table;
            continue;
        }
        inode->i_count++; // 将该 i 节点引用计数增 1
        if (inode->i_mount) {
            int i;
// 如果该 i 节点是其它文件系统的安装电=点,则在超级块表中搜寻安装在此 i 节点的超级块.如果没有找到,
// 则显示出错信息,并释放函数开始获取的空闲节点,返回该 i 节点指针
            for (i = 0; i < NR_SUPER; i++) if (super_block[i].s_imount == inode) break;
            if (i >= NR_SUPER) {
                printk("Mounted inode hasn't got sb\n");
                if (empty) iput(empty);
                return inode;
            }
// 将该 i 节点写盘.从安装在此 i 节点文件系统的超级块上取设备号,并令 i 节点号为 1.然后重新扫描
// 整个 i 节点表,取该被安装文件系统的根节点.
            iput(inode);
...
// 已找到相应的 i 节点,因此放弃临时申请的空闲节点,返回找到的 i 节点
        if (empty) iput(empty);
        return inode;
    }
// 如果在 i 节点中没有找到指定的 i 节点,则利用前面申请的空闲 i 节点在 i 节点表中建立该节点.
// 并从相应设备上读取该 i 节点的信息.返回该 i 节点
...
static void read_inode(struct m_inode *inode) {
    ...
    struct buffer_head *bh;
    int block;
    lock_inode(inode);  // 首先锁定该 i 节点,取该节点所在设备的超级块
...
// 将指定 i 节点信息写入设备(写入缓冲区相应的缓冲块中,带缓冲区刷新时会写入盘中)
static void write_inode(struct m_inode *inode) {
    struct super_block *sb;
    struct buffer_head *bh;
    int block;
// 首先锁定该 i 节点,若该 i 节点没有被修改过或该 i 节点设备号为零,则解锁该 i 节点,并退出
    lock_inode(inode);
    if (!inode->i_dirt || !inode->i_dev) {
        unlock_inode(inode);
        return;
    }
    if (!(sb = get_super(inode->i_dev))) panic("trying to write inode whithout device");
// 该 i 节点所在的逻辑块号=(启动块+超级块)+i 节点位图占用的块数+逻辑块位图占用的块数+(i 节点号-1)/
// 每块含有的 i 节点数
    block = 2 + sb->s_imap_blocks + sb->s_zmap_blocks + (inode->i_num - 1) / INODES_PER_BLOCK;
    if (!(bh = bread(inode->i_dev, block))) panic("unable to read i-node block"); //从设备上读取该 i 节点所在的逻辑块
